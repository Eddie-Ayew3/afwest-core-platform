...
import {
  ...
  LabelComponent, ModalService, AlertDialogService, ToastService,
...
export class LogisticsManagementComponent implements OnInit {
  ...
  approveShipment(shipment: Shipment) {
    ...
    ref.afterClosed$.subscribe(confirmed => {
      ...
      this.applyFilter();
      this.toast.show({ title: 'Shipment Approved', description: `${shipment.trackingNo} has been approved and dispatched.`, variant: 'success' });
    });
  }
  ...
  rejectShipment(shipment: Shipment) {
    ...
    ref.afterClosed$.subscribe(confirmed => {
      ...
      this.applyFilter();
      this.toast.show({ title: 'Shipment Cancelled', description: `${shipment.trackingNo} has been rejected and cancelled.`, variant: 'destructive' });
    });
  }
  ...
  submitShipment() {
    ...
    setTimeout(() => {
      ...
      this.toast.show({ title: 'Shipment Created', description: 'The shipment request has been created and is pending approval.', variant: 'success' });
    }, 600);
  }
