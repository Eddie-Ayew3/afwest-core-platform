...
export class ClientManagementComponent implements OnInit {
  ...
  deleteClient(client: Client) {
    const ref = this.alertDialog.open({
      title: 'Delete Client?',
      description: `Delete client "${client.name}"? All associated sites and records will be removed.`,
      actionText: 'Delete',
      variant: 'destructive'
    });
    ref.afterClosed$.subscribe(confirmed => {
      ...
      this.applyFilter();
      this.toast.show({ title: 'Client Deleted', description: `"${client.name}" has been deleted.`, variant: 'destructive' });
    });
  }
}
