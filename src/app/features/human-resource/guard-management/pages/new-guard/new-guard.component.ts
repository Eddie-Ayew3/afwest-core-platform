...
import { CreateGuardDto } from '../../models/guard.model';
import { selectGuardSaving } from '../../stores/guard.selectors';
...
export class NewGuardComponent implements OnInit {
  ...
  isSubmitting = false;
  saving$ = this.store.select(selectGuardSaving);
  ...
  onSubmit(): void {
    ...
    dialogRef.afterClosed$.subscribe(confirmed => {
      ...
      const formValue = this.guardForm.getRawValue();
      ...
      const dto: CreateGuardDto = {
        ...
        employeeId: formValue.guardId,
        nationalId: formValue.nationalId,
        dateOfBirth: formValue.dateOfBirth ? new Date(formValue.dateOfBirth).toISOString() : undefined,
        hireDate: formValue.startDate ? new Date(formValue.startDate).toISOString() : undefined,
        guardLicense: formValue.licenseNumber,
        licenseExpiry: undefined
      };

      this.store.dispatch(GuardActions.createGuard({ dto }));
      this.toast.show({ title: 'Guard Registered!', description: `${formValue.fullName} has been registered and awaits approval.`, variant: 'success' });
