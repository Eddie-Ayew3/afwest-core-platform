...
export class ZoneManagementComponent implements OnInit {
  ...
  deleteZone(zone: Zone) {
    const ref = this.alertDialog.open({
      title: 'Delete Zone?',
      description: `Are you sure you want to delete "${zone.name}"? This cannot be undone.`,
      actionText: 'Delete',
      variant: 'destructive'
    });
    ref.afterClosed$.subscribe(confirmed => {
      ...
      this.applyFilter();
      this.toast.show({ title: 'Zone Deleted', description: `"${zone.name}" has been deleted.`, variant: 'destructive' });
    });
  }
